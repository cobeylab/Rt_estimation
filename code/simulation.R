...
#' Calculate the cohort reproductive number within the SEIR model
#'
#' Katie Gostic
#' 12 June 2020
#'
...
#' @param t_I Mean duration of infectiousness.
#' @param n_t Number of units of time to simulate.
## Write a function to calculate the cohort reproductive number exactly
## From Walling and Lipsitch (2007), equation 4.2, the cohort reproductive number is the convolution of R instantaneous with the generation interval
get_Rc_ode <- function(
  arnaught,
  t_E,
  t_I,
  ...
  ## Get the fraction susceptible at a given timepoint by solving the SEIR system of equations
  Sfrac <- function(t, t0 = 0, S_0 = S_init){
    inits <- c(S = S_0,   
               E = if(t_E > 0) E_init else 0,
               I = if(t_E > 0) I_init else E_init + I_init,
               R = 0,
    ...
    S <- ode(y = inits,
             times = c(t0,t), 
             func = d_dt_SEIR, parms =NULL)[2,'S']
    S/N
  }
  
  Sref <- ode(y = y_init_SEIR,
              times = c(1:n_t), 
              func = d_dt_SEIR, 
              parms =NULL)[,'S']
  
  ## Set up the ODE for the case reproduction number
  d_dt_Rc <- function(t, y, parms){
    dRc = beta(t)*Sfrac(t, t0 = parms['u'], S_0 = Sref[parms['u']])*(t_I)*dgamma(t-parms['u'], shape = 2, scale = (t_I+t_E))
    list(dRc)
  }
  
  ## Get the max time at which Rc can be calculated accurately
  mt <- qgamma(.99, shape = 2, scale = t_I+t_E)

  ## Integrate from time t to the 99.9th percentile of the generation interval to calculate Rc
  calculate_Rc <- function(t){
    ode(y = c(Rc = 0), times = c(t, t+mt), func = d_dt_Rc, parms = c('u' = t), method = 'ode45')[2,'Rc']
  }
  ...
  Rc <- foreach(tt = 1:(n_t-mt), 
                .packages = 'deSolve',
                .combine = 'c') %dopar% calculate_Rc(tt)
  data.frame(time = 1:(n_t-mt), R_case = Rc)
}


# n_t = 300
# arnaught = c(rep(2, 100), seq(2, 1.1, length = 10), rep(.7, n_t-110+1))
# t_E = 4
# t_I = 4
# N = 1e6
# I_init = 10
...
# ## Test
# Rc <- get_Rc_ode(arnaught = arnaught, 
#                  t_E = t_E, 
#                  t_I = t_I,
#                  N = N, 
#                  E_init = E_init, 
