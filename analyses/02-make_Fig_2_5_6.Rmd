...
cori_wrapper_GI <- function(GI_offset){
  ...
  get_cori(get_sim_df(), icol_name = 'incidence', out_name = sprintf('GI_%.1f', GI), window = wdw, GI_mean = GI)
}

GI_df <- get_sim_df('stochastic') %>%
  ## Merge with true GI fit
...
cori_wrapper_GI_cv <- function(k){
  ...
  get_cori(get_sim_df(), icol_name = 'incidence', out_name = sprintf('k_%.1f', k), window = wdw, GI_mean = mm, GI_var = vv)
}

GI_df_cv <- get_sim_df('stochastic') %>%
  ## Merge with true GI fit
...
GI_df_cv %>%
  pivot_longer(contains('.'), names_to = c('k', '.value'), names_pattern = 'k_(\\d\\d?\\.\\d)\\.(\\w+)') %>%
  mutate(`975` = ifelse(`975`>ymax, ymax, `975`)) %>%
  filter(time < 150) %>%
  mutate(k = factor(k, levels = c('0.1', '0.2', '0.5', '1.0', '2.0'), labels = c('1/8', '1/4', '1/2 (true value)', '1', '2'))) %>%
  ggplot()+
  geom_line(aes(x = time, y = true_rt), lwd = 1)+
  geom_line(aes(x = time, y = mean, color = k))+
  geom_ribbon(aes(x = time, ymin = `025`, ymax = `975`, fill =k), alpha = .1) +
  ylab(expression(paste(R[t])))+
  labs(color = 'Generation interval k', fill = 'Generation interval k') +
  theme(legend.position = c(.75, .7))
