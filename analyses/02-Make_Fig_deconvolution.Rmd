...
data_frame(`t infection` = rep(100, NN)) %>%
        ## Simulate times of observation for each of these 100 infections
  mutate(`t obs` = `t infection` + rdelay(NN),
         ## Infer back the times of infection from the synthetic times of observation
...
gg_png(ww = 7*.8, hh = 6*.65, fn = 'figs/supp_convolution.png', pp = last_plot())
```
